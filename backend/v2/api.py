...
def _dialogue_share_issue(rewritten: str, source: str) -> str:
    ...
    # ① 源稿有对话土壤(≥15%,非纯古风叙事)→ 强推绝对 ≥50%(目标60-70%):靠把叙述/转述/心理判断
    #    对话化(客户R2明确要"加对话/引入角色/把剧情对话化")。纯无对白源稿(古风叙事)豁免,避免逼造假。
    if src_r >= 0.15 and rw_r < 0.50:
        return (
            f'对话占比偏低：洗稿对白占比仅 {rw_r:.0%}，短剧需 ≥60%；'
            '把能说出口的转述、质问、解释、心理判断、信息交代改成带引号直接对白，'
            '必要时让原文已有的对手戏角色开口对话；只把原文已有的事件/人物/台词对话化，'
            '不新增情节、不为凑对话拉长篇幅'
        )
    # ② 高对白源稿被洗低(相对回归)→ 即使已过50%也要补回
    if src_r > 0.35 and rw_r < src_r * 0.85:
        return (
